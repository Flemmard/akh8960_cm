...
static enum msm_camera_vreg_name_t s5k3h1gx_veg_seq[] = {
	CAM_VDIG,
	CAM_VIO,
	CAM_VANA,
	CAM_VAF,
...
static const struct of_device_id s5k3h1gx_dt_match[] = {
	{.compatible = "qcom,s5k3h1gx", .data = &s5k3h1gx_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, s5k3h1gx_dt_match);

static struct platform_driver s5k3h1gx_platform_driver = {
	.driver = {
		.name = "qcom,s5k3h1gx",
		.owner = THIS_MODULE,
		.of_match_table = s5k3h1gx_dt_match,
	},
};

static int32_t s5k3h1gx_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	const struct of_device_id *match;
	match = of_match_device(s5k3h1gx_dt_match, &pdev->dev);
...
{
	int32_t rc = 0;
	rc = platform_driver_probe(&s5k3h1gx_platform_driver,
		s5k3h1gx_platform_probe);
	if (!rc)
		return rc;
...
{
	if (s5k3h1gx_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&s5k3h1gx_s_ctrl);
		platform_driver_unregister(&s5k3h1gx_platform_driver);
	} else
		i2c_del_driver(&s5k3h1gx_i2c_driver);
	return;
}
...
static struct msm_sensor_ctrl_t s5k3h1gx_s_ctrl = {
	...
	.vreg_seq = s5k3h1gx_veg_seq,
	.num_vreg_seq = ARRAY_SIZE(s5k3h1gx_veg_seq),
